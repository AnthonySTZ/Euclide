...
void Node::setInput(const size_t t_index, const std::shared_ptr<Node>& t_sourceNode, const size_t t_sourceIndex) {
    ...
    if (t_index >= m_inputConnections.size())
        return;
    if (t_sourceNode == this_shared)
        return;
    if (t_sourceIndex >= t_sourceNode->m_outputConnections.size())
        return;
    if (t_sourceNode->isInInputsHierarchy(this_shared))
        return;

    deleteInputConnection(t_index);

    auto conn = std::make_shared<NodeConnection>(t_sourceIndex, t_index, t_sourceNode, this_shared);
...
bool Node::isInInputsHierarchy(const std::shared_ptr<Node> t_node) const noexcept {
    for (const auto& conn : m_inputConnections) {
        ...
        if (const auto sourceNode = conn->sourceNode.lock()) {
            if (sourceNode == t_node)
                return true;
            if (sourceNode->isInInputsHierarchy(t_node))
...
void Node::deleteInputConnection(const size_t t_index) {
    ...
    if (auto sourceNode = conn->sourceNode.lock()) {
        ...
        sourceOutputConnections.erase(std::remove(sourceOutputConnections.begin(), sourceOutputConnections.end(), conn),
...
void Node::deleteOutputConnections(const size_t t_index) {
    if (t_index >= m_outputConnections.size())
        return;
...
void Node::addField(const std::string& t_name, std::shared_ptr<NodeFieldBase> t_field) {
    ...
    t_field->onValueChanged.subscribe([this]() { setDirty(); });
