...
class Tokenizer {
  ...
  private:
    inline bool [[nodiscard]] hasMoreTokens() const noexcept { return m_cursor < m_text.length(); }
    inline bool [[nodiscard]] isNum(const char t_letter) const noexcept { return t_letter >= '0' && t_letter <= '9'; }
