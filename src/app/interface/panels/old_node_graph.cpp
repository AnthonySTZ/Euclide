...
// TODO: Use better separation with NodeGraphInputHandler, NodeGraphMenu, NodeGraphRenderer ?

namespace butter {

NodeGraph::NodeGraph(const std::shared_ptr<Scene>& t_scene) : m_scene(t_scene) {
    if (auto scene = m_scene.lock()) {
        scene->onNodeAdded.subscribe([this](uint32_t t_nodeId, std::shared_ptr<Node> t_node) {
            ImGuiIO& io = ImGui::GetIO();
            m_nodeItems.try_emplace(t_nodeId, std::make_shared<NodeItem>(t_node, io.MousePos));
            m_drawOrder.push_back(t_nodeId);
        });

        scene->onNodeRemoved.subscribe([this](uint32_t t_nodeId) {
            if (m_nodeItems.find(t_nodeId) == m_nodeItems.end())
                return;
            m_nodeItems.erase(t_nodeId);
            m_drawOrder.erase(std::remove(m_drawOrder.begin(), m_drawOrder.end(), t_nodeId), m_drawOrder.end());
        });

        scene->onConnectionAdded.subscribe([this](const uint32_t t_sourceId, const uint32_t t_sourceIndex,
                                                  const uint32_t t_destId, const uint32_t t_destIndex) {
        ...
        scene->onConnectionRemoved.subscribe([this](uint32_t t_sourceId, uint32_t t_destId, uint32_t t_destIndex) {
            ...
            m_nodeConnections.erase(std::remove_if(m_nodeConnections.begin(), m_nodeConnections.end(),
                                                   [&](const std::shared_ptr<ConnectionItem>& conn) {
                                                       return conn && conn->sourceNode() == srcNode &&
        ...
        for (auto& [menuName, items] : menuItems) {
            if (m_searchNode.empty()) {
                ...
                for (const auto& item : items) {
                    if (toLower(item.name).find(toLower(m_searchNode)) == std::string::npos)
                        continue;
...
void NodeGraph::searchBar() {
    ...
    if (ImGui::InputTextWithHint("##searchBar", "Search", (char*)m_searchNode.c_str(), m_searchNode.capacity() + 1,
                                 ImGuiInputTextFlags_CallbackResize, StringImGuiCallBack, (void*)&m_searchNode)) {
    }
    ImGui::PopStyleColor();
}
...
void NodeGraph::addConnection(const IOClickedInfos& t_infos) {
    ...
    std::shared_ptr<NodeItem> currentNode =
        currentType == IOType::OUTPUT ? m_currentConnection->sourceNode() : m_currentConnection->destinationNode();
    uint32_t currentIndex =
        currentType == IOType::OUTPUT ? m_currentConnection->sourceIndex() : m_currentConnection->destinationIndex();
...
void NodeGraph::addCuttingLine() {
    ImGuiIO& io = ImGui::GetIO();
    m_cuttingsLines.push(CuttingLine{io.MousePosPrev, io.MousePos});
}
...
void NodeGraph::checkCutConnection() {
    ...
    for (const auto& connection : m_nodeConnections) {
        if (lineIntersect(connection->sourcePosition(), connection->destinationPosition(), io.MousePos,
                          io.MousePosPrev)) {
